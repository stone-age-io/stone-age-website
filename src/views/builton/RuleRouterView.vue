...
const project = {
  name: 'Rule-Router',
  tagline: 'A NATS-native rule engine. Router, HTTP gateway, and cron scheduler — one binary, one YAML syntax.',
  ...
  description: [
    ...
    'Durable state lives in NATS KV, not in the engine — so rules can deduplicate alarms, track presence, and rate-limit actions while the engine itself stays horizontally scalable and trivially restartable.',
  ],
  features: [
    {
      icon: 'pi-share-alt',
  ...
  stoneAge: {
    ...
    points: [
      ...
      'Deploys next to a NATS leaf node at the edge — alerts keep firing with zero cloud dependency.',
